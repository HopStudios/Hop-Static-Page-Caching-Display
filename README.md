#Hop Static Page Caching Display

This plugin allows you to display content on a page only when that page is fetched by the *Static Page Caching* add-on.

That means the content will be displayed only on the cached version of the page.

## Example

```
{exp:hop_static_page_caching_display}
This page was cached on {current_time format="%l, %F %j , %Y, %g:%i %a"}
{/exp:hop_static_page_caching_display}
```
